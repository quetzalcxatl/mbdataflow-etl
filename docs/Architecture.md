...
|---|---|---|---|
| `pipeline_Desinc` | EL | ✅ Producción | Cloud Scheduler diario · 6:00 AM CDMX |
| `pipeline_CanBus` | ETL | 🧪 Funcional local, sin deploy | Manual |
| `pipeline_rangofechas_canbus` | ETL | 🧪 Funcional local, sin deploy | Manual |
...
### Próximo
- Replicar patrón para `pipeline_CanBus`. Lo que cambia respecto a Desinc: paths con `_runtime_path()`, secretos específicos en Secret Manager, propio Job/Scheduler/alerta. Lo que se reutiliza: Dockerfile, SA, cloudbuild.yaml, esquema general.
