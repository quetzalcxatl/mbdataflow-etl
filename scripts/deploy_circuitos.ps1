# ============================================================
# MBDataFlow_ETL — Deploy pipeline-circuitos to Cloud Run
#
# Workflow:
...
#
# Usage:
#   .\scripts\deploy_circuitos.ps1            # Deploy current commit
#   .\scripts\deploy_circuitos.ps1 -Execute   # Also trigger execution after deploy
# ============================================================
...
gcloud run jobs update pipeline-circuitos `
  --image=$IMAGE `
  --region=$REGION
...
Write-Host "    Next scheduled run: Mondays 7:00 AM CDMX (Cloud Scheduler)"
...
if ($Execute) {
    ...
    gcloud run jobs execute pipeline-circuitos --region=$REGION
}