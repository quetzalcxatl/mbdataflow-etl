# ============================================================
# MBDataFlow_ETL — Deploy pipeline-viaje to Cloud Run
#
# Workflow:
...
#
# Usage:
#   .\scripts\deploy_viaje.ps1            # Deploy current commit
#   .\scripts\deploy_viaje.ps1 -Execute   # Also trigger execution after deploy
# ============================================================
...
gcloud run jobs update pipeline-viaje `
  --image=$IMAGE `
  --region=$REGION
...
Write-Host "    Next scheduled run: Mondays 5:00 AM CDMX (Cloud Scheduler)"
...
if ($Execute) {
    ...
    gcloud run jobs execute pipeline-viaje --region=$REGION
}